...
# 4. 建立儲存 Log 的資料夾
LOG_DIR="benchmarks_causal_sim_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
echo "=========================================================="
# Activate conda env groupvit if requested/available
echo "Batch Training Started. Logs: $LOG_DIR"
echo "Cycles: $NUM_CYCLES, Distill/CL: $DISTILL_EPOCHS/$CL_EPOCHS"
...
if [ "$NUM_GPUS" -ge 2 ]; then
    ...
    do
        ...
        if [ -n "$DS1" ]; then
            ...
            echo "[$(date +%T)] Training on $DS1 on GPU 0 (background)..."
            CUDA_VISIBLE_DEVICES=0 uv run python -u -m graph_vit_project.main \
                --dataset "$DS1" \
                --distill \
                --num_cycles "$NUM_CYCLES" \
                --distill_epochs_per_cycle "$DISTILL_EPOCHS" \
                --cl_epochs_per_cycle "$CL_EPOCHS" \
                --eval_mode "$EVAL_MODE" \
        ...
        if [ -n "$DS2" ]; then
            ...
            echo "[$(date +%T)] Training on $DS2 on GPU 1 (background)..."
            CUDA_VISIBLE_DEVICES=1 uv run python -u -m graph_vit_project.main \
                --dataset "$DS2" \
                --distill \
                --num_cycles "$NUM_CYCLES" \
                --distill_epochs_per_cycle "$DISTILL_EPOCHS" \
    ...
    do
        ...
        echo "[$(date +%T)] Training on $DS on GPU 0..."
...
do
    LOG_FILE="$LOG_DIR/${DS}.log"
    if [ -f "$LOG_FILE" ]; then
        # 搜尋具有最高 Val Acc 的列，並提取 Val 和 Test 數值
        # 格式可能為: "[concat] Val Acc: 0.8515, Test Acc: 0.8611"
        RESULT=$(grep "Val Acc:" "$LOG_FILE" | sed 's/,//g' | awk '
        {
            val_acc = 0
            test_acc = 0
            for(i=1; i<=NF; i++) {
